...
import { $, esc, fmtBytes, URLPool, mediaSrc, progressiveImg } from '../core/dom.js';
...
export async function render() {
  ...
  renderGrid();
  renderCounts(source);
  if (document.getElementById('darkroom')?.classList.contains('active')) await renderStudioStats(live);
}
...
function renderGrid() {
  ...
  grid.innerHTML = visible.map((p, i) => {
    ...
    const media = p.demo
      ...
      : progressiveImg(pool, p, { cls: "photo", alt: p.title, eager: i < 3 });
    return `
